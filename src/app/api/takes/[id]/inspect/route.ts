...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const inspection = { ...((take.media_inspection as object | null) || {}), keyframeReview: { reviewedAt: new Date().toISOString(), method: "first_middle_last_stills", referenceIds: references.map((ref) => ref.id), limitations: ["No motion analysis", "No audio analysis", "Unsampled frames were not reviewed"], timestamps, frameUrls, ...review } }
