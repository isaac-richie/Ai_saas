...
export function ProductionReferences({ assets, onChange, disabled = false, onBusy }: {
  ...
  async function upload(file?: File) {
    ...
    if ((!isImage && !isVideo) || file.size > 25 * 1024 * 1024 || !file.size) {
      setError("Choose a JPG, PNG, WebP, MP4, WebM, or MOV file under 25 MB."); return
    }
    ...
    try {
      ...
      onChange([...assets, { id: crypto.randomUUID(), name: file.name.slice(0, 160), role, url: result.url, mediaType: isVideo ? "video" : "image" }])
  ...
  return <details className={styles.drawer} open>
    ...
    {!locked && <div className={styles.controls}>
      ...
      <button type="button" disabled={disabled || uploading || assets.length >= 6} onClick={() => input.current?.click()}>{uploading ? <Loader2 size={16} className="animate-spin" /> : <ImagePlus size={16} />}{uploading ? "Uploading..." : "Add image or video"}</button>
      <input ref={input} type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/quicktime" hidden onChange={event => void upload(event.target.files?.[0])} />
    </div>}
    ...
    <p className={styles.help}>Images guide the visual world. Video references guide motion and pacing; the crew samples their frames when media processing is available. The first character or product image guides the opening video frame.</p>
    {!locked && <p className={styles.help}>JPG, PNG, WebP, MP4, WebM or MOV · Up to 25 MB each. References lock when you save the brief.</p>}
